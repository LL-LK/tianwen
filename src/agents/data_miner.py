...
class DataMiner:
    ...
    async def fetch_exoplanet_data(
        ...
        if not HAS_KEPLER_CLIENT:
            logger.warning("KeplerExoplanetClient不可用，返回空列表")
            return []
    ...
    async def fetch_lightcurve(
        ...
        """
        if not HAS_KEPLER_CLIENT:
            logger.warning("KeplerExoplanetClient不可用，返回空数组")
    ...
    async def search(self, target: str) -> Dict[str, Any]:
        ...
        # 1. 尝试从系外行星数据库搜索
        if HAS_KEPLER_CLIENT:
            try:
                client = KeplerExoplanetClient()
                planets = await client.search_planets(target_name=target)
                if planets:
                    result["found"] = True
                    result["data"]["exoplanets"] = planets
                    result["note"] = "从 NASA Exoplanet Archive 获取的真实数据"
                    return result
            except Exception as e:
                result["note"] = f"系外行星搜索失败: {str(e)}"

        # 2. 尝试从内置星表搜索
        try:
            from observation.sky_chart import BUILTIN_CATALOG, parse_coordinates
            catalog_info = BUILTIN_CATALOG.get(target.upper())
            if catalog_info:
                coords = parse_coordinates(target)
                result["found"] = True
                result["data"]["catalog"] = catalog_info
                if coords:
                    result["data"]["coordinates"] = {"ra": coords[0], "dec": coords[1]}
                result["note"] = "从内置星表获取的信息"
                return result
        except Exception:
            pass

        # 3. 未找到
        result["note"] = f"未找到目标 {target} 的数据"
        return result
...
async def demo():
    ...
    # 打印摘要
    logger.info(miner.get_mining_summary(report))
