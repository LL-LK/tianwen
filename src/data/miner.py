...
try:
    from src.data.kepler_client import KeplerExoplanetClient
...
class DataMiner:
    ...
    def _bootstrap_confidence_interval(self, pattern: DiscoveredPattern) -> Tuple[float, float]:
        ...
        for _ in range(n_iterations):
            # 模拟重采样
            _ = np.random.choice(len(supporting_sources),
    ...
    def _kfold_confidence_interval(self, pattern: DiscoveredPattern) -> Tuple[float, float]:
        ...
        for i in range(n_folds):
            ...
            _ = start_idx + fold_size if i < n_folds - 1 else n_samples
