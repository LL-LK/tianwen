...
class RealBogusClassifier:
    ...
    def load_model(self) -> bool:
        ...
        if not HAS_TORCH:
            logger.info("PyTorch未安装，启用特征工程分类器")
            self._init_feature_classifier()
            return False

        if not os.path.exists(self.weights_path):
            logger.info(f"权重文件不存在: {self.weights_path}，启用特征工程分类器")
            self._init_feature_classifier()
            return False

        if not self._is_valid_weights_file():
            logger.info(f"权重文件无效（全0或损坏）: {self.weights_path}，启用特征工程分类器")
            self._init_feature_classifier()
            return False
        ...
        try:
            ...
            logger.info(f"Real-Bogus分类器(ResNet50)加载成功: {self.weights_path}")
            return True
        except Exception as e:
            logger.info(f"模型加载失败: {e}，启用特征工程分类器")
            self._init_feature_classifier()
            return False
    ...
    async def classify(
        ...
        try:
            ...
            logger.info(f"分类失败: {e}，降级到特征工程分类器")
            return self._fallback_classify(image, return_prob)
