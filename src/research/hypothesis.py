...
class HypothesisGenerator:
    ...
    def generate(self, topic: str, context: str = "") -> List[Hypothesis]:
        """
        同步生成假说的简化接口
        ...
        statement = f"关于{topic}的假说: 如果相关理论成立，那么将观察到特定现象"
        if context:
            statement += f"（背景: {context}）"
        ...
        hypotheses = [
            Hypothesis(
                ...
                status=HypothesisStatus.PENDING,
                source="generated",
                premises=[f"主题: {topic}"] + ([f"背景: {context}"] if context else []),
                predictions=[
                    f"关于{topic}的预测1: 应能观察到特定现象",
                    f"关于{topic}的预测2: 独立验证应能复现相同结果"
                ],
                verification_method="需要进一步观测和文献调研"
            )
        ]
        return hypotheses
...
async def demo():
    """演示假说生成流程"""
    from src.research.literature import LiteratureResearcher
