...
from src.astronomy.pipeline import (
    AstroPipeline,
    StageIDetector,
    SourceDetection
)
from src.observation.enhanced_scheduler import (
...
from src.data.kepler import TransitSignal
...
class TestKeplerExoplanetClientMock(unittest.TestCase):
    ...
    def test_mock_detect_transit_signal(self):
        ...
        time, _ = self.mock_time, self.mock_flux
        self.assertIsNotNone(time)
...
class TestFullObservationLoop(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_full_observation_loop(self):
        ...
        if observation_targets:
            ...
            # 计算可见性窗口
            _ = self.scheduler.compute_operable_periods(
                target, period
            )
